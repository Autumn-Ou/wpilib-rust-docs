...
Static controllers can be used for tasks such as controlling a flywheel while ignoring fluctuating system voltage,
which gives them the name "static"; they are less accurate than simple controllers but easier to set up.
...
let output = feedforward.calculate(position, velocity);
```
